...
def infer(opt, result_path='../results/10x_final/'):
    ...
    adata_pred, val_loss = test_img(adata, model_path, hidden_dims=[opt.h_dim1, opt.h_dim2])
...
def train(opt):
    ...
    #             lambda_1 = opt.g_weight, lambda_2 = opt.i_weight, lambda_3 = opt.c_weight)
    adata_pred, val_loss = train_img(adata, hidden_dims=[opt.h_dim1, opt.h_dim2],  n_epochs=opt.epochs, save_loss=True,
                lr=opt.lr, random_seed=opt.seed, save_path=sp, ncluster=opt.ncluster, 
...
if __name__ == '__main__':
    ...
    logger = open('../result/pruning_10x_parameter2.txt', 'a')
