...
from train_transformer import train_one_branch, test_one_branch
...
def infer(opt, result_path='../results/10x_final/'):
    ...
    adata = test_one_branch(adata, model_path, hidden_dims=[512, 30], branch=opt.branch, use_combine=opt.use_combine, use_img_loss=opt.use_img_loss, random_seed=opt.seed)
...
def train(opt, r):
    ...
    adata = train_one_branch(adata, hidden_dims=[512, 30],  n_epochs=opt.epochs, save_loss=True, 
                lr=opt.lr, random_seed=opt.seed, save_path=sp, ncluster=opt.ncluster, repeat=r, use_combine=opt.use_combine, use_img_loss=opt.use_img_loss, branch=opt.branch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, default='../checkpoint/ablation')
    ...
    parser.add_argument('--branch', type=str, default='gene')
