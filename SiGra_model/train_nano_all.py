...
def gen_adatas(root, id, img_name):
    ...
    # Cal_Spatial_Net(adata, rad_cutoff=120)
...
def main(opt, r=0):
    ...
    # print(os.path.join(opt.root,opt.id, 'CellComposite_%s.jpg'%(opt.img_name)))
    # img = cv2.imread(os.path.join(opt.root,opt.id, 'CellComposite_%s.jpg'%(opt.img_name)))
    # height, width, c = img.shape
    # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    # overlay = cv2.imread(os.path.join(opt.root, opt.id, 'CompartmentLabels_%s.tif'%(opt.img_name)))
    # overlay = cv2.cvtColor(overlay, cv2.COLOR_BGR2GRAY)
    # print(overlay.shape)
