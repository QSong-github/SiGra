...
def infer(opt, r=0, umap_save_path='../results/nano/Lung5-1/figures'):
    ...
    adata, vloss = test_nano_fov(opt, adatas, hidden_dims=[opt.h_dim1, opt.h_dim2],  n_epochs=opt.epochs, save_loss=True,
    ...
    # adata_save_path = '../checkpoint/sigra_emb_nano_%s'%opt.id
    # if not os.path.exists(adata_save_path):
    #     os.makedirs(adata_save_path)
    adata.write('%s/processed_data.h5ad'%(opt.save_path))
    # adata.write('../checkpoint/sigra_emb_nano_%s/processed_data_%s.h5ad'%(opt.id, opt.pretrain[:-4]))
...
def train(opt, r=0):
    ...
    train_nano_fov(opt, adatas, hidden_dims=[opt.h_dim1, opt.h_dim2],  n_epochs=opt.epochs, save_loss=True, 
                lr=opt.lr, random_seed=opt.seed, save_path=sp, ncluster=opt.ncluster, repeat=r)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--h_dim1', type=int, default=512)
    parser.add_argument('--h_dim2', type=int, default=30)
    parser.add_argument('--cluster_method', type=str, default='leiden')
    parser.add_argument('--num_fov', type=int, default=20)
    opt = parser.parse_args()

    logger = open('../result/pruning_nano_grid_search.txt', 'a')
    logger.write('id: %s, g_weight: %.2f, i_weight: %.2f, '%(opt.id, opt.g_weight, opt.i_weight))

    # opt.save_path = os.path.join(opt.save_path, '%.2f_%.2f'%(opt.g_weight, opt.i_weight))
